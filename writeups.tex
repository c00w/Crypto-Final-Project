...
\section{Server Side Verification}
On the server side no verification is done that the received numbers are integers.
\begin{lstlisting}
    float b = (float)atof(info.at(4).c_str());
\end{lstlisting}
Thus if you can impersonate an atm you can send arbitrary floats to the server. Combined with the float exploits described elsewhere you can generate infinite money.
